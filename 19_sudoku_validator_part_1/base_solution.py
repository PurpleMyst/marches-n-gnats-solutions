...
TAPE_SYMBOLS = {*map(str, DIGITS), *ascii_lowercase[: max(DIGITS)], "0", "|"}
SEEN_MARKERS = set(ascii_lowercase[max(DIGITS):][: max(DIGITS)])
...
def seen(d: int) -> str:
    assert d in DIGITS
    return ascii_lowercase[max(DIGITS) + (d - 1)]
...
def main() -> None:
    with Program() as p:
        ...
        p("LEFTWARDS_CLEAN", {*MARKED_DIGITS, *SEEN_MARKERS, ".", "-", "=", "0"} - {mark("0")}, "LEFTWARDS_CLEAN", "_", "L")
        p("LEFTWARDS_CLEAN", "_", "HALT", SAME, "R")
        # Move on to the net row/column/box.
        p.find("ROW_RESTART", "_", {*TAPE_SYMBOLS, *SEEN_MARKERS, "@", "!"} - {*map(str, DIGITS)}, "L", "PRE", "_", "R")
        p.find(
            "COL_RESTART",
            "_",
        ...
        p(f"PRE", SEEN_MARKERS, SAME, "_", "R")
        for s in ("ROW", "COL", "BOX"):
            p(f"PRE", {"ROW": "@", "COL": ",", "BOX": "."}[s], s, SAME, "R")
        p.ignore("PRE", "_", "R")
        ...
        for d in DIGITS:
            ...
            p("ROW", str(d), f"ADD_{d}", mark(d), "L")
            p("COL", str(d), f"ADD_{d}", mark(d), "L")
            p("BOX", str(d), f"ADD_{d}", mark(d), "L")
            p.find(
                f"ADD_{d}",
                "_",
            ...
            p(f"ADD_{d}", seen(d), "PRE_CLEAN", SAME, "L")

            # Stuff we can ignore.
            p.ignore("ROW", {mark(d), "|", "0", mark("=")}, "R")
            p.ignore("COL", {*MARKED_DIGITS, "|"}, "R")
            p.ignore("BOX", {mark(d), "=", "0", mark(mark("|"))}, "R")
            ...
            p.find(
                "BOX_NL",
                "=",
                {*TAPE_SYMBOLS, *SEEN_MARKERS} - set(ascii_lowercase) - {"$", "="},
                "R",
                "BOX",
                SAME,
                "R",
            )
            p.ignore("RET", {*SEEN_MARKERS}, "R")
            p("RET", "@", "ROW", SAME, "R")
            p("RET", ",", "COL_NL", SAME, "R")
            p("RET", ".", "BOX", SAME, "R")
            ...
            p("PREPARE_FOR_BOX", {"="}, "PREPARE_FOR_BOX_1", "=", "R")
            p("PREPARE_FOR_BOX_1", {"="}, "PREPARE_FOR_BOX_2", "=", "R")
            p("PREPARE_FOR_BOX_2", {"="}, "PREPARE_FOR_BOX", "$", "R")
            p.ignore({"PREPARE_FOR_BOX", "PREPARE_FOR_BOX_1", "PREPARE_FOR_BOX_2"}, {"|"}, "R")
            p.ignore({"PREPARE_FOR_BOX"}, {".", "|"}, "R")
            p({"PREPARE_FOR_BOX_2"}, "_", "RETURN_FOR_BOX", SAME, "L")
            p.find("RETURN_FOR_BOX", ".", {*map(str, DIGITS), "0", "|", "$", "="} - {"."}, "L", "BOX", SAME, "R")
